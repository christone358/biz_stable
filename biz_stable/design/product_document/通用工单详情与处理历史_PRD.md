...
- 说明：办理模式不再重复出现“备注/附件”，避免与模板字段产生重复填写

### 5.3 模式一：办理（动态表单）
- 目的：仅填写工单类型对应的办理差异化字段；无额外备注/附件输入
...
### 14.2 布局结构
- 顶部状态统计卡片：横向铺满可用宽度（随容器自适应换行）。卡片含“处置中｜已完成｜已作废｜全部”等统计，点击切换当前状态视图。
- 筛选条：位于统计卡片下方，包含状态视图、任务类型、时间范围、关键词等过滤器，并显示当前筛选摘要。
...
### 14.10 验收要点
- 列表项仅保留“优先级 Tag、任务类型 Tag、标题、编号、创建时间、发起人、当前处理人、时间字段（完成/截止）”；不出现“责任单位/上一环节审批人/描述/进度条”。
